...
| 입력 | 기대 결과 |
| --- | --- |
| `/analyze-repo-for-kubernetes` | Target 질문 한 번, workspace 탐색 command 없음 |
...
각 실행에서 질문 UI의 개수와 command event를 기록한다. Target 없는 첫 시나리오는 skill/AGENTS bootstrap read는 허용하지만, workspace path를 대상으로 한 `rg`, `find`, `ls`, `git` discovery가 완료되면 실패다. Codex hosted web 도구는 `PreToolUse` hook 범위 밖이므로, web event가 발생하지 않았는지도 별도로 확인한다.
